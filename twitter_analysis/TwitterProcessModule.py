...
class TwitterAuthentication(CloudAuthentication):
    ...
    def __get_credential(self, parameters):
        ...
        #self.__twitter_auth_path_file = str(Path(__file__).parent) + os.path.sep + parameters.at[0,'Path'] + parameters.at[0,'File']
        self.__twitter_auth_path_file = os.path.join(str(Path(__file__).parent) ,  parameters.at[0,'Path'] , parameters.at[0,'File'])
        #if os.path.isfile(self.__twitter_auth_path_file ):
        #    print("Ici")

        #if not isinstance(url, (str, self.__twitter_auth_path_file)):
        #    raise TypeError("Argument 'url' must be a string or path-like object")
        tmp_file = open(self.__twitter_auth_path_file , "r")
        self.__df_twitter_auth = pd.read_json(path_or_buf = tmp_file, orient='records')
        #self.__df_twitter_auth = pd.read_json(path_or_buf = self.twitter_auth_path_file, orient='records')
...
class TwitterProcess:
    ...
    def get_search_terms(self):
        ...
        searchTerms = ["#edchat", "#K12","#learning","#edleadership","#edtech",
                        "#engchat","#literacy","#scichat","#mathchat","#edreform",
                        "Teacher2Teacher"]
...
class GeneralDbHelper():
    ...
    def __init__(self, version):
        self.version = version
    def get_store_dir(self,store_dir_name,file_name):
        social_network_path_file = os.path.join(str(Path(__file__).parent) , store_dir_name , file_name)
        return social_network_path_file